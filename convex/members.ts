...
export const update = mutation({
  ...
  handler: async(ctx, args) => {
    ...
    if(currentMember.role !== 'admin') throw new Error('Unauthorized')
...
export const remove = mutation({
  ...
  handler: async(ctx, args) => {
    ...
    if(currentMember._id === args.id && currentMember.role === 'admin') throw new Error('Cannot remove itself, if it is the only admin')
